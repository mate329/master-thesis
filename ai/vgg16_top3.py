...
# Load a pre-trained VGG16 model and add a softmax layer
def create_vgg16_with_softmax(num_classes):
...
model_x = create_vgg16_with_softmax(NUM_CLASSES)
model_y = create_vgg16_with_softmax(NUM_CLASSES)
model_z = create_vgg16_with_softmax(NUM_CLASSES)

# Modify the classifier to match the number of classes
model_x.classifier[6] = nn.Linear(model_x.classifier[6].in_features, NUM_CLASSES)
model_y.classifier[6] = nn.Linear(model_y.classifier[6].in_features, NUM_CLASSES)
model_z.classifier[6] = nn.Linear(model_z.classifier[6].in_features, NUM_CLASSES)
...
def plot_and_save_confusion_matrix(conf_matrix, class_names, axis):
    ...
    save_path = f'./confusion_matrix_{axis}_vgg16.png'
...
def train_model(model, optimizer, train_loader, valid_loader, criterion, early_stopping, axis_label):
        ...
        for phase in ['train', 'valid']:
            ...
            for inputs, labels in dataloader:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    topk_vals, topk_indices = torch.topk(outputs, 3, dim=1)
                    topk_corrects = torch.any(topk_indices == torch.max(labels, 1)[1].unsqueeze(1), dim=1)
                ...
                running_corrects += torch.sum(topk_corrects)
            ...
            recall = recall_score(all_labels, [item[0] for item in all_preds], average='weighted', zero_division=0)
            f1 = f1_score(all_labels, [item[0] for item in all_preds], average='weighted', zero_division=0)
    ...
    conf_matrix = confusion_matrix(all_labels, [item[0] for item in all_preds])
...
# Save models
torch.save(model_x.state_dict(), 'enterpin_vgg_top3_model_x.pth')
torch.save(model_y.state_dict(), 'enterpin_vgg_top3_model_y.pth')
torch.save(model_z.state_dict(), 'enterpin_vgg_top3_model_z.pth')

# Function to save arrays to CSV
def save_arrays_to_csv(axis_label, train_losses, valid_losses, train_accuracies, valid_accuracies, train_recalls, valid_recalls, train_f1s, valid_f1s):
    ...
    save_path = f'./enterpin_metrics_{axis_label.lower()}_vgg16_top3.csv'
...
def plot_metrics(axis_label, train_losses, valid_losses, train_accuracies, valid_accuracies, train_recalls, valid_recalls, train_f1s, valid_f1s):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_loss_vgg16.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_accuracy_vgg16.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_recall_vgg16.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_f1_score_vgg16.png'), dpi=300)
    plt.close()
