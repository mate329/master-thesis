...
# Modify the final layer to match the number of classes with Sequential Layer including Softmax
model_x.fc = nn.Sequential(
    nn.Linear(model_x.fc.in_features, NUM_CLASSES),
    nn.Softmax(dim=1)
)
model_y.fc = nn.Sequential(
    nn.Linear(model_y.fc.in_features, NUM_CLASSES),
    nn.Softmax(dim=1)
)
model_z.fc = nn.Sequential(
    nn.Linear(model_z.fc.in_features, NUM_CLASSES),
    nn.Softmax(dim=1)
)
        ...
        for phase in ['train', 'valid']:
            ...
            for inputs, labels in dataloader:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # Calculate accuracy
                    corrects = torch.any(topk_indices == torch.max(labels, 1)[1].unsqueeze(1), dim=1).sum().item()
...
# Save models
torch.save(model_x.state_dict(), 'enterpinactivity_resnet_top3_model_x.pth')
torch.save(model_y.state_dict(), 'enterpinactivity_resnet_top3_model_y.pth')
torch.save(model_z.state_dict(), 'enterpinactivity_resnet_top3_model_z.pth')
...
def save_arrays_to_csv(axis_label, train_losses, valid_losses, train_accuracies, valid_accuracies, train_recalls, valid_recalls, train_f1s, valid_f1s):
    ...
    save_path = f'./enterpinactivity_metrics_{axis_label.lower()}_resnet_top3.csv'
...
def plot_metrics(axis_label, train_losses, valid_losses, train_accuracies, valid_accuracies, train_recalls, valid_recalls, train_f1s, valid_f1s):
    ...
    save_dir = './enterpinactivity_plots_resnet'
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpinactivity_{axis_label}_loss_resnet.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpinactivity_{axis_label}_accuracy_resnet.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpinactivity_{axis_label}_recall_resnet.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpinactivity_{axis_label}_f1_score_resnet.png'), dpi=300)
    plt.close()
