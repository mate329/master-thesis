...
model_x = models.vgg16(weights=models.VGG16_Weights.DEFAULT)
model_y = models.vgg16(weights=models.VGG16_Weights.DEFAULT)
model_z = models.vgg16(weights=models.VGG16_Weights.DEFAULT)
...
def plot_and_save_confusion_matrix(conf_matrix, class_names, axis):
    ...
    save_path = f'./confusion_matrix_{axis}_vgg16_top1.png'
...
# Save models
torch.save(model_x.state_dict(), 'vgg16_top1_model_x.pth')
torch.save(model_y.state_dict(), 'vgg16_top1_model_y.pth')
torch.save(model_z.state_dict(), 'vgg16_top1_model_z.pth')
...
def save_arrays_to_csv(axis_label, train_losses, valid_losses, train_accuracies, valid_accuracies, train_recalls, valid_recalls, train_f1s, valid_f1s):
    ...
    save_path = f'./enterpin_metrics_{axis_label.lower()}_vgg16_top1.csv'
...
def plot_metrics(axis_label, train_losses, valid_losses, train_accuracies, valid_accuracies, train_recalls, valid_recalls, train_f1s, valid_f1s):
    ...
    save_dir = './enterpin_plots_vgg16_top1'
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_loss_vgg16_top1.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_accuracy_vgg16_top1.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_recall_vgg16_top1.png'), dpi=300)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'enterpin_{axis_label}_f1_score_vgg16_top1.png'), dpi=300)
    plt.close()
